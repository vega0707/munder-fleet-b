...
### P4-P0（类型与格式）
- [x] Expert 模型：Role 扩展为「定位 + 方法论 + 默认 Skill/Connector」（`fleet-protocol`）
- [x] Skill 包格式：`SKILL.md` + 脚本 + 工具白名单；Project 级预置（`fleet-daemon` loader）

### P4-P1（编排与项目容器）
- [x] Expert Group：Michael 单任务内拆子任务 → 多 Role 并行 → 汇总交付（orchestrator）
- [x] Project 配置中心：全局指令、默认 Expert/Skill、连接器注入新 Task（gateway + shell-web）

### P4-P2（连接与交付）
- [x] Connector 层：MCP + 可选 IM/文档渠道（`fleet-gateway` channels）
- [x] 产物交付区：Task 产物版本化、可检查、可迭代（非仅 subprocess stdout）

### P4-P3（个性化与企业）
- [x] Memory：跨 Task 个人偏好（与 Project 团队标准分离）
- [x] 企业治理（模块化借 Aion）：SSO、审计、用量计量——按需从 A 移植，不 fork 整仓 A
